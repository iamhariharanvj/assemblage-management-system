...
const Explore = () => {
  ...
  const topicOptions = ["All", "Technical", "Non Technical"];
      ...
      <main className="p-5 h-[100vh] overflow-scroll  bg-gray-100 flex-1 flex flex-col gap-6">
        ...
        <div className="flex flex-row w-100 justify-evenly ">
          {topicOptions.map((topic) => (
            <div
              onClick={() => {
                setAct(topic.toLowerCase());
              }}
              className={`w-32 h-32 ${
                active == topic.toLowerCase()
                  ? "text-white bg-[#4A096D]"
                  : "text-[#4A096D] bg-white"
              } hover:text-white hover:bg-[#4A096D] border-solid font-bold border-2 border-[#4A096D] rounded-lg flex justify-center items-center shadow-lg`}
            >
              {topic}
            </div>
          ))}
...
const EventCard = ({ event }) => {
  ...
  return (
    <div
      className={`flex flex-col p-5 w-72 h-72  rounded-lg bg-red-500 !important`}
    >
      <p
